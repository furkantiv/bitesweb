import { Category } from "@/utils/types";

export const categories: Category[] = [
  {
    name: "Training and Simulation Systems",
    slug: "trainingsimulation",
    image: "/images/products/categories/Training and Simulation Systems.png",
    items: [
      {
        name: "Embedded Simulations",
        image: "/images/products/karma.png",
        slug: "embedded-simulations",
        items: [],
      },
      {
        name: "Troubleshooting and Maintenance Simulation Systems",
        image: "/images/products/bites-lms-lxp.png",
        slug: "troubleshooting-maintenance-simulation",
        items: [],
      },
      {
        name: "Mission Simulators",
        image: "/images/products/immersive-xr.png",
        slug: "mission-simulators",
        items: [],
      },
      {
        name: "Mixed Reality Simulation & Training Systems",
        image: "/images/products/embedded-training-system.png",
        slug: "mixed-reality-simulation-training",
        items: [],
      },
      {
        name: "Digital Assessment Systems",
        image: "/images/products/augmented-reality-content-creator.png",
        slug: "digital-assessment-systems",
        items: [],
      },
    ],
  },
  {
    name: "Defence Information Systems",
    slug: "defenceinformation",
    image: "/images/products/categories/Defence Systems.png",
    items: [
      {
        name: "Atok",
        image: "/images/products/atok.png",
        slug: "atok",
        items: [],
      },
      {
        name: "Porsuk",
        image: "/images/products/porsuk.png",
        slug: "porsuk",
        items: [],
      },
      {
        name: "Bright Sight",
        image: "/images/products/BrightSight.png",
        slug: "bright-sight",
        items: [],
      },
      {
        name: "ALTAY Data Management System",
        image: "/images/products/altay.png",
        slug: "altay-data-management",
        items: [],
      },
      {
        name: "Augmented Reality Sand BOX",
        image: "/images/products/sand-box.png",
        slug: "arsbox",
        items: [],
      },
      {
        name: "Video Management System",
        image: "/images/products/video-management-system.png",
        slug: "video-management",
        items: [],
      },
    ],
  },
  {
    name: "Technology and Business Solutions Systems",
    slug: "technologyandbusiness",
    image:
      "/images/products/categories/Technology and Business Solutions Systems.png",
    items: [
      {
        name: "Digital Twin",
        image: "/images/products/digital-twin.png",
        slug: "digital-twin",
        items: [],
      },
    ],
  },
  {
    name: "Avionics and Mission Systems",
    slug: "avionicsandmission",
    image: "/images/products/categories/Avionics and Mission Systems.png",
    items: [
      {
        name: "Digital Twin",
        image: "/images/products/avionic-system-software.png",
        slug: "avionics-systems-software",
        items: [],
      },
    ],
  },
  // diğer kategoriler...
];
